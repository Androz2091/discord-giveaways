...
class GiveawaysManager extends EventEmitter {
    ...
    start(channel, options) {
        return new Promise(async (resolve, reject) => {
            ...
            if (
                !options.isDrop &&
                (isNaN(options.duration) || typeof options.duration !== 'number' || options.duration < 1)
            ) {
            ...
            const giveaway = new Giveaway(this, {
                ...
                bonusEntries: Array.isArray(options.bonusEntries)
                ...
                lastChance:
                    options.lastChance && typeof options.lastChance === 'object' ? options.lastChance : undefined,
                pauseOptions:
                    options.pauseOptions && typeof options.pauseOptions === 'object' ? options.pauseOptions : undefined,
                allowedMentions:
                    ...
                    .awaitReactions({
                        filter: (r, u) =>
                            [r.emoji.name, r.emoji.id]
                                .filter(Boolean)
                                .includes(reaction.emoji.name || reaction.emoji.id) && u.id !== this.client.user.id,
    ...
    _checkGiveaway() {
        ...
        this.giveaways.forEach(async (giveaway) => {
            ...
            if (giveaway.isDrop) {
                ...
                reaction = giveaway.message?.reactions.cache.find((r) =>
            ...
            if (giveaway.pauseOptions.isPaused) {
                if (
                    (isNaN(giveaway.pauseOptions.unPauseAfter) ||
                        typeof giveaway.pauseOptions.unPauseAfter !== 'number') &&
                    (isNaN(giveaway.pauseOptions.durationAfterPause) ||
                        typeof giveaway.pauseOptions.durationAfterPause !== 'number')
                ) {
